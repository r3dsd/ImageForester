...
class MiddleLayout(QHBoxLayout):
    ...
    def _initSignal(self):
        ...
        GUISearchSignalManager().on_search_completed.connect(self.searched_list.on_search_completed)

    def _on_item_selection_updated(self, image_data: ImageFileData):
        GUISearchSignalManager().emit_on_item_selection_updated(image_data)
        self.image_viewer.update_image(image_data.file_path)

    def _on_select_list_saved(self, mode):
        GUISearchSignalManager().emit_on_select_list_saved(mode)

    def _on_deleted_data(self, int):
        self.image_viewer.clear()
        GUISearchSignalManager().emit_on_deleted_data(int)